...
function runSimulation(inp) {
    ...
    let hours = 0;
    const serviceTimeSteps = Math.ceil(inp.serviceTime * inp.precision);
...
self.onmessage = function (e) {
    ...
    if (inputs.arrivalRate * inputs.serviceTime / 3600 > inputs.spaces) {
        self.postMessage("Error: Arrival rate too high for available spaces.");
    } else {
        try {
            const results = runSimulation(inputs);
            self.postMessage(formatResults(results));
        } catch (err) {
